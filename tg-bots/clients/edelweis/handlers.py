...
from clients.edelweis.keyboards import example_buttons
from clients.edelweis.conversator_edelweis import ConversatorEdelweis
...
edelweis_conversator = ConversatorEdelweis(
    table_id=None,
    text_id=s.edelweis.reglament,
    name='edelweis',
    model='gpt-4o-mini')

edelweis_router = Router()


@edelweis_router.message(CommandStart())
...
@edelweis_router.message(F.text == 'Выслать прайс')
@forward_and_send_message(s.edelweis.channel_id, s.edelweis.manager_ids, edelweis_conversator)
async def query_options(message: Message, bot: Bot):
    ...
    await message.answer_document(FSInputFile(path='logs/edelweis/price_list.xlsx'))
    return {"reply": reply}


@edelweis_router.message(Command('update'))
async def update_instructions(message: Message):
    ...
    Args:
        message (Message): telegram message object
    """
    edelweis_conversator.update()
    await message.answer('Инструкции обновлены.')


@edelweis_router.message(F.content_type == ContentType.DOCUMENT)
async def handle_document(message: Message, bot: Bot):
    ...
    Args:
        message (Message): telegram message object
    """
    if message.from_user.id not in s.edelweis.admin_ids:
    ...
    if document.mime_type == 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet':
        ...
        with open("logs/edelweis/price_list.xlsx", "wb") as f:
            f.write(file.read())
        try:
            edelweis_conversator.update()
        except:
...
@edelweis_router.message(Command('model'))
...
@edelweis_router.message(ModelUpdate.model)
async def set_model(message: Message, state: FSMContext):
    # await state.update_data(model=message.text)
    # data = await state.get_data()
    edelweis_conversator.change_model(message.text.split(' ', maxsplit=1)[0])
    await message.answer(f'Установлена модель {getattr(edelweis_conversator, "model")}',
                         reply_markup=await extended_kb(example_buttons))
    await state.clear()


@edelweis_router.message()
@forward_and_send_message(s.edelweis.channel_id, s.edelweis.manager_ids, edelweis_conversator)
async def generate_reply_message(message: Message, bot: Bot):
    ...
    try:
        async with action_sender:
            ...
            result = await edelweis_conversator.generate_reply(message)
        ...
        await report_error(message=message,
                           bot=bot,
                           bot_name=getattr(edelweis_conversator, 'path')[5:],
