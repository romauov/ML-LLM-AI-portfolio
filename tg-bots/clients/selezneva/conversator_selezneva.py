...
from clients.selezneva.file_loader import load_files
...
from utils.conversator.tools import call_manager, generate_tools, get_prices, pick_products
from utils.openai_client.client import client


class ConversatorSelezneva(Conversator):
    ...
    def update(self):
        ...
        """
        self.price_list, self.reglament = load_files(self.table_id, self.text_id, self.path)
    ...
    async def generate_reply(self, message: Message):
        ...
        products_list = self.price_list['product'].unique().tolist()
        messages = [
        ...
        tools = generate_tools('Получение цен и веса упаковки для списка продуктов')
