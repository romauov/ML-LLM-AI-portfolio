...
from clients.torgkit.file_loader import load_files
...
class ConversatorTorgKit(Conversator):
    ...
    async def generate_reply(self, message: Message):
        ...
        products_list = self.price_list['Продукция Изготовитель'].unique(
        ).tolist()
        messages = [
            ...
            for tool_call in tool_calls:
                ...
                if function_to_call:
                    ...
                    if function_name == 'get_prices':
                        picked_products, prompt_tokens, completion_tokens = await pick_products(client=client,
                                                                                                 model=self.model,
                                                                                                 products_list=products_list,
                                                                                                 user_promt=message.text)
                        print(picked_products)
                        ...
                        function_response = get_prices(df=self.price_list,
                                                       ...
                                                       product_col_name='Продукция Изготовитель')
