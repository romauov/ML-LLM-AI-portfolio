...
from clients.torgkit.keyboards import example_buttons
from clients.torgkit.conversator_torgkit import ConversatorTorgKit
...
torgkit_conversator = ConversatorTorgKit(
    table_id=None,
    text_id=s.torgkit.reglament,
    name='torgkit',
    model='gpt-4o-mini')

torgkit_router = Router()


@torgkit_router.message(CommandStart())
async def send_welcome(message: Message):
    ...
    """
    await message.answer("Здравствуйте! Я эксперт компании \"КИТ\". Готов ответить на вопросы. Что вас интересует?",
                         reply_markup=await extended_kb(example_buttons))


@torgkit_router.message(F.text == 'Выслать прайс')
@forward_and_send_message(s.torgkit.channel_id, s.torgkit.manager_ids, torgkit_conversator)
async def query_options(message: Message, bot: Bot):
    ...
    await message.answer_document(FSInputFile(path='logs/torgkit/torgkit_pricelist.pdf'))
    return {"reply": reply}


@torgkit_router.message(Command('update'))
async def update_instructions(message: Message):
    ...
    Args:
        message (Message): telegram message object
    """
    torgkit_conversator.update()
    await message.answer('Инструкции обновлены.')


@torgkit_router.message(F.content_type == ContentType.DOCUMENT)
async def handle_document(message: Message, bot: Bot):
    ...
    Args:
        message (Message): telegram message object
    """
    if message.from_user.id not in s.torgkit.admin_ids:
    ...
    if document.mime_type == 'application/pdf':
        ...
        with open("logs/torgkit/torgkit_pricelist.pdf", "wb") as f:
            f.write(file.read())
        try:
            torgkit_conversator.update()
        except:
            await message.answer("Пожалуйста отправьте файл формата PDF")
        await message.answer("Прайс-лист обновлён.")
    else:
        await message.answer("Пожалуйста, отправьте файл в формате PDF.")


@torgkit_router.message(Command('model'))
...
@torgkit_router.message(ModelUpdate.model)
async def set_model(message: Message, state: FSMContext):
    # await state.update_data(model=message.text)
    # data = await state.get_data()
    torgkit_conversator.change_model(message.text.split(' ', maxsplit=1)[0])
    await message.answer(f'Установлена модель {getattr(torgkit_conversator, "model")}',
                         reply_markup=await extended_kb(example_buttons))
    await state.clear()


@torgkit_router.message()
@forward_and_send_message(s.torgkit.channel_id, s.torgkit.manager_ids, torgkit_conversator)
async def generate_reply_message(message: Message, bot: Bot):
    ...
    try:
        async with action_sender:
            ...
            result = await torgkit_conversator.generate_reply(message)
        ...
        await report_error(message=message,
                           bot=bot,
                           bot_name=getattr(torgkit_conversator, 'path')[5:],
