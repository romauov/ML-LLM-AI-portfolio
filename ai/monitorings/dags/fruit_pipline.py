...
CONTAINER_FRUIT_NEW_MONS = os.path.join(CONTAINER_MONS_NEW, 'fruit')
CONTAINER_FRUIT_SUCCESS_MONS = os.path.join(CONTAINER_MONS_PROCESSED, 'fruit')
CONTAINER_FRUIT_ERROR_MONS = os.path.join(CONTAINER_MONS_ERRORS, 'fruit')
FRUIT_MONS_TYPE = 'fruit'
...
@dag(
    ...
    schedule="0 * * * *",
    params={
        "source_folder": CONTAINER_FRUIT_NEW_MONS,
        "success_destination": CONTAINER_FRUIT_SUCCESS_MONS,
        "failure_destination": CONTAINER_FRUIT_ERROR_MONS,
...
def excel_pipline():
    wait_file_checker = PythonSensor(
        ...
        op_args=[CONTAINER_FRUIT_NEW_MONS, EXCEL_EXTENSIONS],
    ...
    collect_files_task = PythonOperator(
        ...
        op_args=[CONTAINER_FRUIT_NEW_MONS, EXCEL_EXTENSIONS], do_xcom_push=True,
    )
        ...
        **dockerops_kwargs
        ...
        command=collect_files_task.output.map(
            lambda file_path: f'python -m app.docker_commands.make_process_file '
                              f'--source_file_path {file_path} --monitoring_type {FRUIT_MONS_TYPE}'
        )
    )
        ...
        **dockerops_kwargs
        ...
        command=extract_excel_task.output.map(
            lambda args: f'python -m app.docker_commands.make_save_results '
                         ...
                         f'--monitoring_type {FRUIT_MONS_TYPE}'
        )
    )
