...
from constants import HOST_TMP, HOST_MONS_NEW, HOST_MONS_PROCESSED, HOST_MONS_ERRORS, CONTAINER_TMP, \
    CONTAINER_MONS_NEW, CONTAINER_MONS_PROCESSED, CONTAINER_MONS_ERRORS, EXCEL_EXTENSIONS, SENSOR_TIMEOUT, \
    CHECK_INTERVAL
from utils import check_files_exist, collect_files, on_success_move_file, on_failure_move_file, clean_up_temp_files

CONTAINER_FISH_NEW_MONS = os.path.join(CONTAINER_MONS_NEW, 'fish')
CONTAINER_FISH_SUCCESS_MONS = os.path.join(CONTAINER_MONS_PROCESSED, 'fish')
CONTAINER_FISH_ERROR_MONS = os.path.join(CONTAINER_MONS_ERRORS, 'fish')
FISH_MONS_TYPE = 'fish'
...
@dag(
    ...
    schedule="0 * * * *",
    params={
        "source_folder": CONTAINER_FISH_NEW_MONS,
        "success_destination": CONTAINER_FISH_SUCCESS_MONS,
        "failure_destination": CONTAINER_FISH_ERROR_MONS,
...
def excel_pipline():
    wait_file_checker = PythonSensor(
        ...
        op_args=[CONTAINER_FISH_NEW_MONS, EXCEL_EXTENSIONS],
    ...
    collect_files_task = PythonOperator(
        ...
        op_args=[CONTAINER_FISH_NEW_MONS, EXCEL_EXTENSIONS], do_xcom_push=True,
    )
        ...
        **dockerops_kwargs
        ...
        command=collect_files_task.output.map(
            lambda file_path: f'python -m app.docker_commands.make_process_file '
                              f'--source_file_path {file_path} --monitoring_type {FISH_MONS_TYPE}'
        )
    )
        ...
        **dockerops_kwargs
        ...
        command=extract_excel_task.output.map(
            lambda args: f'python -m app.docker_commands.make_save_results '
                         ...
                         f'--monitoring_type {FISH_MONS_TYPE}'
        )
    )
