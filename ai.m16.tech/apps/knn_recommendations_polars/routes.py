...
@blueprint.route('/knn_recommenations_polars', methods=['POST'])
def rec():
    """
    ...
    if isinstance(result, dict):
        log = f"{request_data['product']} " + f"tsop_id: {request_data['tsop_id']} " + "ОШИБКА! " + result['error']
        logger.info(log)
        return json.dumps(result, ensure_ascii=False)

    if len(result[0]) == 0:
        log = f"{request_data['product']} " + f"tsop_id: {request_data['tsop_id']} " + "Вернулся пустой массив!"
        logger.info(log)
        return json.dumps({"error": f"Отправлен пустой список пользователей. Продукт: {request_data['product']}, "
                                    f"id клиента: {request_data['tsop_id']}"},
                                    ensure_ascii=False)
