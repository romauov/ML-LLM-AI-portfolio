...
from knowledge.parsing_piplines.vic_articles_avian.constants import (
    JSON_ARTICLES, IMAGES_PATH, MAX_CONTENT_CHARS,
)
...
def process_articles():
    ...
    # Локальные файлы называются {prefix_статьи}_{server_filename}, поэтому
    # server_filename извлекается через rsplit('_', 1)
    ...
    for article in articles:
        ...
        if len(content) > MAX_CONTENT_CHARS:
            print(f"[SKIP] Слишком большой контент ({len(content)} символов): '{title[:70]}'")
            skipped_large.append(title)
            continue

        # 2. Пропускаем уже загруженные статьи
        ...
        for section in sections:
            ...
            for sub_chunk in sub_chunks:
                ...
                # Ищем ссылки на изображения в чанке
                image_matches = re.findall(r'!\[.*?\]\((.*?)\)', chunk_content)

                # Убираем разметку изображений из текста
                clean_content = re.sub(r'!\[.*?\]\(.*?\)', '', chunk_content).strip()
                if not clean_content:
                    clean_content = f"Изображение из статьи '{title}'"

                chapter_title = sub_chunk.metadata.get(
                    'Header 2', sub_chunk.metadata.get('Header 1', title)
                )
                ...
                try:
                    ...
                    for img_ref in image_matches:
                        server_filename = os.path.basename(img_ref)
