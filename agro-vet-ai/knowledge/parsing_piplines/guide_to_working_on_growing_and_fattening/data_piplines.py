...
from knowledge.parsing_piplines.guide_to_working_on_growing_and_fattening.constants import SOURCE_DOCUMENT, CHAPTERS, \
...
def add_source():
    """Добавление сущности `источник документа` в базу данных по книге Guide to working on growing and fattening"""
...
def add_chunks():
    ...
    splitter = BookSplitter(
        pages=pages,
        table_description_regex='Таблица \d{1,3}\.\d{1,3}:',
        image_description_regex='Рисунок \d{1,3}\.\d{1,3}:',
        to_drop_line_regex='',
    )
    ...
    for chunk_number, chunk in enumerate([*texts, *tables, *images], start=1):
        print(chunk_number, _get_chapter(chunk['page_number']), chunk)
