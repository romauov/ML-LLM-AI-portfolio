...
from knowledge.parsing_piplines.streptococcosis_dissertation.constants import SOURCE_DOCUMENT, CHAPTERS, JSON_BOOK, \
...
def add_chunks():
    ...
    splitter = BookSplitter(
        pages=pages,
        table_description_regex='(?<!\()(Таблица|Продолжение таблицы) \d{1,3}',
    ...
    for chunk_number, chunk in enumerate([*texts, *tables, *images], start=1):
        ...
        if chapter_title == '2.2 Результаты собственных исследований':
            chunk['type'] = 'examples'
...
def add_images():
    ...
    stmt = select(KnowledgeBaseChunk.id).where(
        ...
        (
                ...
                ((KnowledgeBaseChunk.content_type == 'examples') & (KnowledgeBaseChunk.content_name.like('%Рисунок%')))
        )
