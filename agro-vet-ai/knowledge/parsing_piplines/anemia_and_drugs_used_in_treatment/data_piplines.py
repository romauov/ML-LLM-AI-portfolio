...
from knowledge.parsing_piplines.anemia_and_drugs_used_in_treatment.constants import SOURCE_DOCUMENT, CHAPTERS, \
    JSON_BOOK, EXCLUDED_PAGES
...
def add_chunks():
    ...
    splitter = BookSplitter(
        pages=pages,
        table_description_regex='(?<!\()Таблица \d',
    )
    ...
    for chunk_number, chunk in enumerate([*texts, *tables], start=1):
        try:
