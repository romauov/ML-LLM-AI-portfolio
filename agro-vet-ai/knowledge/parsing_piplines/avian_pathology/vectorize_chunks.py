...
from knowledge.parsing_piplines.avian_pathology.constants import JSON_BOOK, CHAPTERS, EXCLUDED_PAGES, SOURCE_DOCUMENT
from knowledge.parsing_piplines.avian_pathology.utils import fix_page_headers, split_composite_tables
...
def process_chunks():
    ...
    pages = fix_page_headers(pages, CHAPTERS)

    print(f"\n{'='*80}")
    print("ОБРАБОТКА КНИГИ С ПОМОЩЬЮ BOOK SPLITTER:")
    print(f"{'='*80}\n")

    # Инициализация BookSplitter с параметрами для русскоязычной книги
    splitter = BookSplitter(
        pages=pages,
        table_description_regex=r'^Table\s+\d+|^#\s+Birds Frequently Kept as Pets',
        image_description_regex=r'^Fig\s*\.\s*\d+:',
        text_chunk_size=2000
    )

    # Pipeline обработки согласно лучшим практикам из README
    ...
    tables = split_composite_tables(tables)
