...
from app.db.sqlalchemy_models import SourceDocument, KnowledgeBaseChunk
...
from knowledge.parsing_piplines.atrophic_rhinitis_of_pigs.constants import SOURCE_DOCUMENT, CHAPTERS, JSON_BOOK, \
    EXCLUDED_PAGES
...
def add_source():
    """Добавление сущности `источник документа` в базу данных по книге Атрофический ринит свиней"""
...
def add_chunks():
    ...
    for chunk_number, chunk in enumerate(texts, start=1):
        chapter_title = _get_chapter(chunk['page_number'])
        if chunk_number == 8:
            chapter_title = 'ВВЕДЕНИЕ'
        if chunk_number == 22:
            chapter_title = 'ДИАГНОСТИЧЕСКИЕ МЕТОДЫ'
