...
from app.agents.router_v2.pharmacist_utils import (
    format_drug_chunks,
    build_drug_summary_list,
)
...
def search_by_trade_name(trade_name: str, section_type: Optional[str] = None) -> dict:
    """
    ...
    if exact_match or len(matching_drugs) == 1:
        ...
        if section_types:
            ...
            chunks = _search_engine.query_executor.get_all_chunks_for_drug(
                selected_name
            )
        ...
        return {"text": formatted_text, "documents": chunks}
    ...
    return {"text": clarification_text, "documents": matching_drugs}


@tool(parse_docstring=True)
def search_by_active_substance(
    generic_name: str, animal: Optional[str] = None, section_type: Optional[str] = None
) -> dict:
    """
    ...
    if len(unique_drugs) > cfg.pharmacist.search.overflow_threshold:
        ...
        return {"text": summary, "documents": chunks}
...
def search_by_conditions(
    ...
    if not chunks:
        return {"text": "По вашему запросу препараты не найдены.", "documents": []}
