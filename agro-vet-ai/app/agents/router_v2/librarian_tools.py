...
from app.agents.router_v2.librarian_utils import (
    format_context,
    find_table_and_figure_names,
    memory_view_to_base64_str,
)
...
def search_database_by_all_books(question_ru: str, question_en: str) -> dict:
    ...
    for question, language in [(question_ru, "ru"), (question_en, "en")]:
        ...
        images.extend(
            [memory_view_to_base64_str(d["image"]) for d in documents if d.get("image")]
        )
...
def get_page_content(page_numbers: list[int], source_name: str) -> dict:
    ...
    if tables_names:
        tables = search_engine.get_tables_chunk(
            tables_names=tables_names, source_name=source_name
        )
        documents.extend(tables)
    if figures_names:
        figures = search_engine.get_figures_chunk(
            figures_names=figures_names, source_name=source_name
        )
