...
def extract_substances_and_build_footnote(
    ...
    from app.agents.router.prompts import EXTRACT_SUBSTANCES_PROMPT

    messages = [
        {"role": "user", "content": EXTRACT_SUBSTANCES_PROMPT.format(
...
def format_drug_chunks(chunks: list[dict]) -> str:
    ...
    # Группируем чанки по препаратам
    drugs_map = {}
    for chunk in chunks:
    ...
    for trade_name, data in drugs_map.items():
        ...
        if data['target_animals']:
            animals = ', '.join(data['target_animals']) if isinstance(data['target_animals'], list) else data['target_animals']
        ...
        for section in unique_sections:
            section_title = section.get('section_title', section.get('section_type', 'Информация'))
            content = section.get('content', '') or ''

            # Убираем дублирование контекстного заголовка из content
            # (он добавляется при создании чанков)
            lines = content.split('\n')
            filtered_lines = []
            skip_header = True
            for line in lines:
                if skip_header and (
                    line.startswith('Препарат:') or
                    line.startswith('Действующее вещество:') or
...
def format_drug_list(drugs: list[dict]) -> str:
    ...
    Args:
        drugs: Список словарей с метаданными препаратов
    ...
    for i, drug in enumerate(drugs, 1):
        ...
        animals_str = ', '.join(target_animals) if isinstance(target_animals, list) else target_animals
...
def build_drug_summary_list(chunks: list[dict]) -> str:
    ...
    Returns:
        Отформатированная строка со списком препаратов
    """
    if not chunks:
        return "Препараты не найдены."
    ...
    for i, (trade_name, data) in enumerate(drugs_seen.items(), 1):
        animals_str = ', '.join(data['target_animals']) if isinstance(data['target_animals'], list) else str(data['target_animals'])
        parts.append(
            f"{i}. {trade_name} ({data['generic_name']}) — {data['drug_class']}"
            + (f", животные: {animals_str}" if animals_str and animals_str != '-' else "")
        )
